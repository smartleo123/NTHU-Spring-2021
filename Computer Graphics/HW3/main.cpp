...
void refresh_memorydata(int shapesNum, int ShadingType) {
	float x = 0;
	float y = 0;
	glUniform1i(iLocIsEye, models[cur_idx].shapes[shapesNum].material.isEye);
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
		...
		refresh_memorydata(i, per_vertex_or_per_pixel);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_I:
			...
			break;
		case GLFW_KEY_RIGHT:
			models[cur_idx].cur_eye_offset_idx == 6 ? models[cur_idx].cur_eye_offset_idx = 0 : models[cur_idx].cur_eye_offset_idx = models[cur_idx].cur_eye_offset_idx + 1;
			break;
		case GLFW_KEY_LEFT:
			models[cur_idx].cur_eye_offset_idx == 0 ? models[cur_idx].cur_eye_offset_idx = 6 : models[cur_idx].cur_eye_offset_idx = models[cur_idx].cur_eye_offset_idx - 1;
			break;
		case GLFW_KEY_G:
			cur_magn = (cur_magn + 1) % 2;
			break;
		case GLFW_KEY_B:
			cur_min = (cur_min + 1) % 2;
			break;
...
void LoadTexturedModels(string model_path)
{
	...
	for (int i = 0; i < materials.size(); i++)
	{
		...
		if (string(materials[i].diffuse_texname).find("Eye") != -1) {
			material.isEye = 1;
...
void setUniformVariables()
{
	...
	iLocDiffuseTex = glGetUniformLocation(program, "diffuseTexture");
	...
	iLocEyeoffsetY = glGetUniformLocation(program, "eyeOffset.y");
	iLocLightPosition = glGetUniformLocation(program, "light.position");
	iLocLightDiffuse = glGetUniformLocation(program, "light.diffuse");
	iLocShadingType = glGetUniformLocation(program, "ShadingType");
	...
	iLocCutoffAngle = glGetUniformLocation(program, "light.angle");
}
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109065534 HW3", NULL, NULL);
    if (window == NULL)
    {
