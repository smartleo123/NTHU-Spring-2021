...
GLfloat save_window_width = WINDOW_WIDTH;
GLfloat save_window_height = WINDOW_HEIGHT;
...
void ChangeSize(GLFWwindow* window, int width, int height)
{
	...
	proj.aspect = (float)width / height / 2;
...
void RenderScene(void) {	
	...
	{
		glViewport(0, 0, save_window_width / 2, save_window_height);
		//refresh_memorydata(i, 0);
		phong_model(i, view_matrix, 0);
		glBindVertexArray(models[cur_idx].shapes[i].vao);
		glDrawArrays(GL_TRIANGLES, 0, models[cur_idx].shapes[i].vertex_count);

		glViewport(save_window_width / 2, 0, save_window_width / 2, save_window_height);
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case lightedit:
		if (cur_light_mode == 0) {
			Light.directional.intensity += Vector3(0.1 * yoffset, 0.1 * yoffset, 0.1 * yoffset);
		}
		if (cur_light_mode == 1) {
			Light.point.intensity += Vector3(0.1 * yoffset, 0.1 * yoffset, 0.1 * yoffset);
		}
		if (cur_light_mode == 2) {
			Light.spot.angle += (0.3 * yoffset);
		}
		break;
	case shineedit:
		shininess -= yoffset;
	...
	if (mouse_pressed == true) {
		...
		case GeoTranslation:
			models[cur_idx].position +=
				Vector3(0.005 * (xpos - starting_press_x), -0.005 * (ypos - starting_press_y), 0);
			break;
		case GeoScaling:
		...
		case GeoRotation:
			models[cur_idx].rotation +=
				Vector3(0.005 * (ypos - starting_press_y), 0.005 * (xpos - starting_press_x), 0);
			break;
		case ViewEye:
...
void setupRC()
{
	...
	for (int cur_idx = 0; cur_idx < 5; cur_idx++) {
		LoadModels(model_list[cur_idx]);
		cout << model_list[cur_idx] << endl;
	}
}
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109065534 HW2", NULL, NULL);
    if (window == NULL)
    {
