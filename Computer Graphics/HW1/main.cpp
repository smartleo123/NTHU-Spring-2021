...
void setPerspective()
{
	...
	/*
	project_matrix[0] = (2 * proj.nearClip) / (proj.right - proj.left);
	project_matrix[1] = project_matrix[3] = 0;
	project_matrix[2] = -(proj.right + proj.left) / (proj.right - proj.left);
	project_matrix[4] = project_matrix[7] = 0;
	project_matrix[5] = (2 * proj.nearClip) / (proj.top - proj.bottom);
	project_matrix[6] = -(proj.top + proj.bottom) / (proj.top - proj.bottom);
	project_matrix[8] = project_matrix[9] = 0;
...
void RenderScene(void) {	
	...
	*/
	...
	glPolygonOffset(-1, -1);
	glPolygonMode(GL_FRONT_AND_BACK, isDrawWireframe ? GL_LINE : GL_FILL);
	...
	if (mouse_pressed == true) {
		...
		case GeoRotation:
			models[cur_idx].rotation +=
				Vector3(0.01 * (ypos - starting_press_y), 0.01 * (xpos - starting_press_x), 0);
			break;
		case ViewEye:
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109065534 HW1", NULL, NULL);
    if (window == NULL)
    {
